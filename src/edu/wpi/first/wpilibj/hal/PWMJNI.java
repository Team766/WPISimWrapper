...
public class PWMJNI extends DIOJNI {
	private static boolean[] usedPorts = new boolean[SensorBase.kPwmChannels];

	public static boolean allocatePWMChannel(long digital_port_pointer) {
		System.out.println("PWMJNI: allocatePWMChannel()");
		if (!usedPorts[(int) digital_port_pointer]) {
			usedPorts[(int) digital_port_pointer] = true;
			return true;
		}
		return false;
	}
	...
	public static void setPWM(long digital_port_pointer, double value) {
		...
		if (digital_port_pointer == Ports.PWM_Left_Drive) {
			VRConnector.getInstance().putCommandFloat(VRConnector.LEFT_MOTOR,
					(float) value);
			// System.out.println("Left Drive: " + value);
		} else if (digital_port_pointer == Ports.PWM_Right_Drive) {
			VRConnector.getInstance().putCommandFloat(VRConnector.RIGHT_MOTOR,
					(float) value);
	...
	public static void setPWMPeriodScale(long digital_port_pointer,
			int squelchMask) {
